...
<div class="row">
    <div class="col-md-12">
        <h3 class="" style="font-weight: 900;text-align: center;margin-bottom: 30px;max-width: 70%;margin-left: auto;margin-right: auto;">Chỉnh sửa <a href="{{ route('ui.article.detail-audio', $article->slug) }}">{{$article->title}}</a></h3>
    </div>
    {!!Form::open(array( 'route' => ['ui.article.edit-audio-update', $article->slug], 'method' => 'POST', 'class' => 'form-horizontal', 'files' => true ))!!}
    <div class="col-md-9">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                <input type="text" readonly="true" class="form-control" name="slug" id="inputPassword3" placeholder="Slug sẽ hiển thị trên thanh address trình duyệt" value="{{$article->slug}}">
            </div>
        </div>
        <div class="form-group">
            <label for="iAuth" class="col-sm-2 control-label">Tác giả</label>
            <div class="col-sm-10">
                <input type="text" class="form-control @if(!empty($errors->first('auth'))) error-box @endif" name="auth" id="iAuth" placeholder="Tác giải bài hát..." value="{{$article->author}}">
                @if($errors->count() > 0)<span class="error">{{$errors->first('auth')}}</span>@endif
            </div>
        </div>
        <div class="form-group">
            <label for="iDesc" class="col-sm-2 control-label">Mô tả</label>
            <div class="col-sm-10">
                <textarea name="description" id="iDesc" class="form-control @if(!empty($errors->first('description'))) error-box @endif" rows="3"  placeholder="Nhập mô tả ngắn gọn về bài viết...">{{$article->description}}</textarea>
        ...
        <div class="form-group">
            <label for="iAudio" class="col-sm-2 control-label">Tải nhạc</label>
            <div class="col-sm-10">
                <input type="file" class="form-control @if(!empty($errors->first('audio'))) error-box @endif" id="iAudio" name="audio" value="{{old('audio')}}">
                 @if($errors->has('audio'))<span class="error">{{$errors->first('audio')}}</span>@endif
                 <input type="hidden" name="old_audio" value="{{$article->thumbnail}}">
            </div>
        </div>
    </div>
    <div class="col-md-3">
